...
const core_engine_1 = require("@error-debugger/core-engine");
async function main() {
    ...
    const engine = new core_engine_1.ErrorDebuggerEngine();
    // Detect error type heuristics
    let source = 'runtime';
    if (errorText.includes('TS') || errorText.includes('SyntaxError'))
        source = 'compile';
    if (errorText.includes('git') || errorText.includes('push') || errorText.includes('merge'))
        source = 'git';
    const rawEvent = {
        id: `cli-${Date.now()}`,
        source,
