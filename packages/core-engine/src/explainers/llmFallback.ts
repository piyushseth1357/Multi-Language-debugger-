...
export class LLMFallback {
  ...
  public async explain(rawError: RawErrorEvent, config: DebuggerConfig): Promise<Explanation> {
...
${rawError.codeContext || 'N/A'}`;
  ...
  private async callGeminiAPI(systemPrompt: string, userPrompt: string, config: DebuggerConfig): Promise<string> {
  ...
  private async callOpenAICompatibleAPI(systemPrompt: string, userPrompt: string, config: DebuggerConfig): Promise<string> {
  ...
  private buildFallbackOfflineExplanation(rawError: RawErrorEvent, language: SupportedLanguage, note: string): Explanation {
