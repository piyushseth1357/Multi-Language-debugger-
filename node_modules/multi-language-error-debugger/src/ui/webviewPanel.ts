...
import { Explanation, SupportedLanguage, RawErrorEvent, DebuggerConfig } from '@error-debugger/shared-types';
import { ErrorDebuggerEngine } from '@error-debugger/core-engine';

export class ErrorDebuggerWebviewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'errorDebuggerView';
  private _view?: vscode.WebviewView;
  private currentExplanations: Explanation[] = [];
  private lastRawErrors: RawErrorEvent[] = [];
  private currentLanguage: SupportedLanguage = 'hi';
  private totalDetectedCount = 0;
  private totalFixedCount = 0;
  ...
  public updateExplanation(explanation: Explanation, rawError?: RawErrorEvent) {
    this.updateExplanations([explanation], rawError ? [rawError] : []);
  }

  public updateExplanations(explanations: Explanation[], rawErrors: RawErrorEvent[] = []) {
  ...
  public async refreshLanguage(engine: ErrorDebuggerEngine, config: DebuggerConfig) {
    if (this.lastRawErrors.length > 0) {
      const newExps: Explanation[] = [];
  ...
  private async showMultiLanguageApiKeyGuide() {
  ...
  private async applyFixToEditor(fixCode: string, filePath?: string, lineNo?: number) {
    if (!fixCode) return;
  ...
  private async applyBatchFixes(items: Array<{ code: string; file: string; line: number }>) {
